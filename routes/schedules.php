...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::prefix('schedules')->middleware('permission:view_schedules')->group(function () {
        ...
        Route::middleware('permission:manage_schedules')->group(function () {
            ...
            Route::post('/assign-employees', [ScheduleController::class, 'assignEmployees'])->name('schedules.assign-employees');
            Route::delete('/assignments/{schedule}', [ScheduleController::class, 'removeAssignment'])->name('schedules.remove-assignment');
            Route::post('/import', [ScheduleController::class, 'import'])->name('schedules.import');
        });
        
        // AJAX/DataTable routes
        Route::prefix('data')->group(function () {
            Route::get('/periods', [ScheduleController::class, 'getPeriodsData'])->name('schedules.periods.data');
            Route::get('/calendar', [ScheduleController::class, 'getCalendarData'])->name('schedules.calendar.data');
            Route::get('/periods/{period}/assignments', [ScheduleController::class, 'getPeriodAssignments'])->name('schedules.period.assignments');
        });
        
        // Template download
        Route::get('/download-template', [ScheduleController::class, 'downloadTemplate'])->name('schedules.download-template');
    });
    
    // Academic Schedule Management (for schools)
    Route::prefix('academic')->middleware('permission:view_schedules')->group(function () {
        Route::get('/schedules', [AcademicScheduleController::class, 'index'])->name('academic.schedules.index');
        ...
        Route::prefix('api')->group(function () {
            Route::get('/teachers-with-subjects', [AcademicScheduleController::class, 'getTeachersWithSubjects']);
            Route::get('/schedules/grid/{classId}', [AcademicScheduleController::class, 'getScheduleGrid']);
            ...
            Route::middleware('permission:lock_schedules')->group(function () {
                Route::post('/schedules/{schedule}/lock', [AcademicScheduleController::class, 'lockSchedule']);
                Route::delete('/schedules/{schedule}/lock', [AcademicScheduleController::class, 'unlockSchedule']);
            });
            
            Route::middleware('permission:resolve_conflicts')->group(function () {
                Route::post('/schedules/conflicts/{conflict}/resolve', [AcademicScheduleController::class, 'resolveConflict']);
            });
        });
    });
});