...
const mockHoliday = {
  ...
  date: '2024-04-10',
  type: 'religious',
  description: 'Hari Raya Idul Fitri 1445 Hijriyah',
  is_recurring: true,
};
...
export default function HolidayEditPage() {
  ...
  const [isRecurring, setIsRecurring] = useState(mockHoliday.is_recurring);
  ...
  const {
    ...
    defaultValues: {
      ...
      type: mockHoliday.type,
      description: mockHoliday.description,
      is_recurring: mockHoliday.is_recurring,
    },
  });
  ...
  const onSubmit = async (data: HolidayForm) => {
    setIsLoading(true);
    try {
      console.log('Updating holiday:', { ...data, is_recurring: isRecurring });
