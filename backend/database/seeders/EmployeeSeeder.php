...
class EmployeeSeeder extends Seeder
{
    public function run()
    {
        ...
        if ($users->count() < 10) {
            $additionalUsers = [
                ['name' => 'John Smith', 'email' => 'john.smith@school.edu', 'role' => 'guru'],
                ['name' => 'Sarah Johnson', 'email' => 'sarah.johnson@school.edu', 'role' => 'guru'],
                ['name' => 'Michael Brown', 'email' => 'michael.brown@school.edu', 'role' => 'guru'],
                ['name' => 'Lisa Wilson', 'email' => 'lisa.wilson@school.edu', 'role' => 'guru'],
                ['name' => 'David Miller', 'email' => 'david.miller@school.edu', 'role' => 'guru'],
                ['name' => 'Emma Davis', 'email' => 'emma.davis@school.edu', 'role' => 'pegawai'],
                ['name' => 'James Garcia', 'email' => 'james.garcia@school.edu', 'role' => 'pegawai'],
                ['name' => 'Maria Rodriguez', 'email' => 'maria.rodriguez@school.edu', 'role' => 'pegawai'],
            ];
        ...
        foreach ($users as $user) {
            ...
            $employee = Employee::create([
                ...
                'metadata' => json_encode([
                    ...
                    'address' => rand(100, 9999).
                      ' '.
                      ['Main St', 'Oak Ave', 'Pine Rd', 'Elm Dr', 'Maple Way'][
                        array_rand(['Main St', 'Oak Ave', 'Pine Rd', 'Elm Dr', 'Maple Way'])
                      ],
                    'city' => ['Springfield', 'Riverside', 'Franklin', 'Georgetown', 'Madison'][
                      array_rand(['Springfield', 'Riverside', 'Franklin', 'Georgetown', 'Madison'])
                    ],
                    'state' => ['CA', 'NY', 'TX', 'FL', 'IL'][array_rand(['CA', 'NY', 'TX', 'FL', 'IL'])],
                    'zip_code' => rand(10000, 99999),
                    'emergency_contact_name' => ['Alice Smith', 'Bob Johnson', 'Carol Brown', 'David Wilson'][
                      array_rand(['Alice Smith', 'Bob Johnson', 'Carol Brown', 'David Wilson'])
                    ],
                    'emergency_contact_phone' => '+1-555-'.rand(100, 999).'-'.rand(1000, 9999),
                    'department' => $departments[array_rand($departments)],
                    'position' => $type === 'honorary'
                        ? 'Honorary Teacher'
                    ...
                    'qualifications' => [
                        'degree' => ['Bachelor\'s', 'Master\'s', 'PhD'][
                          array_rand(['Bachelor\'s', 'Master\'s', 'PhD'])
                        ],
                        'certifications' => rand(0, 1) ? ['Teaching License', 'CPR Certified'] : [],
                    ],
                    'face_recognition' => rand(0, 10) < 8
                        ? [
                            // 80% have face recognition set up
                            'registered_at' => Carbon::now()->subDays(rand(1, 30))->toISOString(),
                            'descriptor' => base64_encode(
                                json_encode(array_fill(0, 128, rand(-100, 100) / 100)),
                            ), // Fake face descriptor
            ...
            $this->command->info(
                "Created employee: {$employee->full_name} ({$employee->employee_id}) - {$employee->employee_type}",
            );
        }
    }
}
