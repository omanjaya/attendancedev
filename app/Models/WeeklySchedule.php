...
class WeeklySchedule extends Model
{
    ...
    {
        return $this->hasMany(ScheduleConflict::class, 'schedule_id_1')
                    ->orWhere('schedule_id_2', $this->id);
    }
    ...
    public function getStatusAttribute()
    {
        ...
        if ($this->hasActiveConflicts()) return 'conflict';
        return 'normal';
    }
    ...
    public function isEffectiveOn($date = null)
    {
        ...
        if ($this->effective_from > $date) return false;
        if ($this->effective_until && $this->effective_until < $date) return false;
    ...
    public static function getGridData($academicClassId, $date = null)
    {
        ...
        foreach (self::DAYS_OF_WEEK as $dayKey => $dayName) {
            ...
            foreach ($timeSlots as $slot) {
                $schedule = $schedules->where('day_of_week', $dayKey)
