...
class PayrollReportController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('pages.payroll.reports.index', compact(
            'currentMonthSummary',
            'ytdSummary',
            'recentPayrolls',
            'currentMonth',
    ...
    public function summary(Request $request)
    {
        ...
        $payrolls = $query->orderBy('payroll_period_start')
            ->orderBy('employee_id')
            ->get();
    ...
    public function taxReport(Request $request)
    {
        ...
        foreach ($payrolls as $payroll) {
            $taxableIncome = $payroll->payrollItems()
                ->where('is_taxable', true)
                ->sum('amount');

            $taxDeducted = $payroll->payrollItems()
                ->where('category', 'tax')
                ->sum('amount');

            $statutoryDeductions = $payroll->payrollItems()
    ...
    protected function generatePdfSummary($summary)
    {
        ...
        return view('pages.payroll.reports.pdf.summary', compact('summary'))
            ->with('isPdf', true);
    }
    ...
    protected function generateExcelSummary($summary)
    {
        ...
        $filename = 'payroll_summary_' . $summary['period_start']->format('Y-m-d') . '_to_' . $summary['period_end']->format('Y-m-d') . '.csv';
        ...
        $callback = function() use ($summary) {
            ...
            fputcsv($file, ['Period', $summary['period_start']->format('Y-m-d') . ' to ' . $summary['period_end']->format('Y-m-d')]);
            ...
            fputcsv($file, ['Total Gross Salary', '$' . number_format($summary['total_gross_salary'], 2)]);
            ...
            fputcsv($file, ['Employee ID', 'Employee Name', 'Period', 'Gross Salary', 'Deductions', 'Bonuses', 'Net Salary', 'Status']);
    ...
    {
        return view('pages.payroll.reports.pdf.detailed', compact('reportData'))
            ->with('isPdf', true);
    }
    ...
    {
        $filename = 'payroll_detailed_' . $reportData['period_start']->format('Y-m-d') . '_to_' . $reportData['period_end']->format('Y-m-d') . '.csv';
        ...
        $callback = function() use ($reportData) {
            ...
            fputcsv($file, ['Period', $reportData['period_start']->format('Y-m-d') . ' to ' . $reportData['period_end']->format('Y-m-d')]);
            fputcsv($file, ['Generated', $reportData['generated_at']->format('Y-m-d H:i:s')]);
            fputcsv($file, []);
            
            foreach ($reportData['payrolls'] as $payroll) {
                fputcsv($file, ['Employee', $payroll->employee->full_name . ' (' . $payroll->employee->employee_id . ')']);
    ...
    {
        return view('pages.payroll.reports.pdf.comparison', compact('comparison'))
            ->with('isPdf', true);
    }
    ...
    {
        return view('pages.payroll.reports.pdf.employee-history', compact('historyData'))
            ->with('isPdf', true);
    }
    ...
    {
        $filename = 'employee_payroll_history_' . $historyData['employee']->employee_id . '_' . $historyData['year'] . '.csv';
        ...
        $callback = function() use ($historyData) {
            ...
            fputcsv($file, ['Period', 'Gross Salary', 'Deductions', 'Bonuses', 'Net Salary', 'Worked Hours', 'Overtime Hours', 'Status']);
            ...
            fputcsv($file, []);
            fputcsv($file, ['Summary']);
            fputcsv($file, ['Total Gross Salary', '$' . number_format($historyData['summary']['total_gross_salary'], 2)]);
            fputcsv($file, ['Total Deductions', '$' . number_format($historyData['summary']['total_deductions'], 2)]);
            fputcsv($file, ['Total Bonuses', '$' . number_format($historyData['summary']['total_bonuses'], 2)]);
            fputcsv($file, ['Total Net Salary', '$' . number_format($historyData['summary']['total_net_salary'], 2)]);
            fputcsv($file, ['Average Monthly Gross', '$' . number_format($historyData['summary']['average_monthly_gross'], 2)]);
            fputcsv($file, ['Average Monthly Net', '$' . number_format($historyData['summary']['average_monthly_net'], 2)]);
    ...
    {
        return view('pages.payroll.reports.pdf.tax', compact('taxData'))
            ->with('isPdf', true);
    }
    ...
    {
        $filename = 'tax_report_' . $taxData['period_start']->format('Y-m-d') . '_to_' . $taxData['period_end']->format('Y-m-d') . '.csv';
        ...
        $callback = function() use ($taxData) {
            ...
            fputcsv($file, ['Period', $taxData['period_start']->format('Y-m-d') . ' to ' . $taxData['period_end']->format('Y-m-d')]);
            fputcsv($file, ['Generated', now()->format('Y-m-d H:i:s')]);
            fputcsv($file, []);
            
            fputcsv($file, ['Summary']);
            fputcsv($file, ['Total Taxable Income', '$' . number_format($taxData['total_taxable_income'], 2)]);
            fputcsv($file, ['Total Tax Deducted', '$' . number_format($taxData['total_tax_deducted'], 2)]);
            fputcsv($file, ['Total Statutory Deductions', '$' . number_format($taxData['total_statutory_deductions'], 2)]);
            fputcsv($file, []);
            
            fputcsv($file, ['Employee ID', 'Employee Name', 'Period', 'Taxable Income', 'Tax Deducted', 'Statutory Deductions', 'Net Salary']);
