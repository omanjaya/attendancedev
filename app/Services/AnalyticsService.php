...
class AnalyticsService
{
    ...
    public function getKPIs($startDate, $endDate)
    {
        ...
        $punctualityRate =
          $presentRecords > 0 ? (($presentRecords - $lateRecords) / $presentRecords) * 100 : 0;
        $absenteeismRate =
          $totalAttendanceRecords > 0 ? ($absentRecords / $totalAttendanceRecords) * 100 : 0;
        ...
        $leaveApprovalRate =
          $leaveRequests->count() > 0 ? ($approvedLeaves / $leaveRequests->count()) * 100 : 0;
        ...
        $payrollProcessingRate =
          $payrolls->count() > 0 ? ($processedPayrolls / $payrolls->count()) * 100 : 0;
    ...
    public function getLeaveOverview($startDate, $endDate)
    {
        ...
        $leaveTypeDistribution = $leaveData->groupBy('leaveType.name')->map(function ($leaves) {
            return [
                'count' => $leaves->count(),
    ...
    public function getRealTimeAnalytics()
    {
        ...
        $recentActivity = Attendance::with('employee')
            ->where(function ($query) {
                $query
                    ->where('check_in_time', '>=', Carbon::now()->subMinutes(30))
